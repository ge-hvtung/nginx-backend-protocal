...
type NgxParser interface {
	...
	ParseUpstreams() ([]httpupstream.Upstream, error)
...
func (p *NgxParserImpl) ParseUpstreams() ([]httpupstream.Upstream, error) {
	// Init parser
	conf := p.InitParse()
	...
	ngxUpstreams := make([]httpupstream.Upstream, 0)

	// Loop through upstreams
	for _, upstream := range upstreams {
		// use httpupstream.ParseUpstreamBlock
		// Create a new upstream
		ngxUpstream, err := httpupstream.ParseUpstreamBlock(upstream.GetBlock())
